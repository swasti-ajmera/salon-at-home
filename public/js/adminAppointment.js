document.addEventListener("DOMContentLoaded", () => {
    ...
    function updateStats(appointments) {
      ...
      document.getElementById("pending-count").textContent = appointments.filter(
        (app) => app.status === "Pending"
      ).length;
      document.getElementById("approved-count").textContent = appointments.filter(
        (app) => app.status === "Approved"
      ).length;
      document.getElementById("rejected-count").textContent = appointments.filter(
    ...
    function renderAppointments(appointments) {
      ...
      appointments.forEach((appointment) => {
        ...
        row.innerHTML = `
                  ...
                  <td><span class="status ${appointment.status.toLowerCase()}">${
          appointment.status
        }</span></td>
                  <td>
                      ${
    ...
    window.approveAppointment = async (appointmentId) => {
      try {
        ...
        fetchAppointments();
        M.toast({
          html: "Appointment approved successfully",
          classes: "green",
        });
      } catch (error) {
    ...
    document
      .getElementById("submit-rejection")
      .addEventListener("click", async () => {
        ...
        if (!rejectionReason.trim()) {
          M.toast({
            html: "Please provide a reason for rejection",
            classes: "red",
          });
          return;
        }
        ...
        try {
          ...
          M.toast({
            html: "Appointment rejected successfully",
            classes: "orange",
          });
        } catch (error) {
