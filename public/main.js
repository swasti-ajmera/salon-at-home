document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderSalons(salons) {
    ...
    container.innerHTML = salons
      .map(
        (salon) => `
            <div class="salon-card" data-name="${salon.name}" data-suburb="${salon.suburb}">
                <div class="salon-image" style="background-image: url(
                  https://media.istockphoto.com/id/1423513079/photo/luxury-hairdressing-and-beauty-salon-interior-with-chairs-mirrors-and-spotlights.jpg?s=612x612&w=0&k=20&c=T2cw6HblRzWPOlxPHsMcV9KRV8aLGOaLojvLe3Yoi_A=
                 )"></div>
                <div class="salon-card-content">
                    <h5 class="salon-card-title">${salon.name}</h5>
                    <p class="salon-info">${salon.description}</p>
                    <p class="salon-info-add"> Address : ${salon.addressLine}</p>
                    <p class="salon-info-sub"> Suburb : ${salon.suburb}</p>
